function [SysParam] = curr_Nov07_RefSiO2_2MHz()

%% DATA FILE NAME

SysParam.filename = 'Z:\ForMATLAB\fits\RefSiO2_80nmAl_2MHz@2MHz_011123_214921_FIN.mat';

%% SAMPLE PROPERTIES (starting with top layer )
% Al / interface / SiO2 (1 um) / interface / Si:
SysParam.Labels = ["Al", "Al / SiO2", "SiO2 (1um)", "SiO2/Si", "Si (substr.)"]; % Labels for each element in the lists below. Only used in SIM plots

SysParam.Lambda  = [237 0.08 1.3 0.08 124];  % Thermal conductivities (W m^-1 K^-1)
SysParam.C       = [2.42 2.42 1.86 2.42 1.63]*1e6;  % Volumetric heat capacities (J m^-3 K^-1)
SysParam.h       = [80 1 1000 1 1e6]*1e-9;  % Thicknesses (m)  
...
SysParam.FITNLambda = [1 2 3 4 5]; % [2 3 4]
SysParam.FITNC = [1 3 5]; 
SysParam.FITNh = [1];
