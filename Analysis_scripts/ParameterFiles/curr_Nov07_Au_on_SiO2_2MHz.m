function [SysParam] = curr_Nov07_Au_on_SiO2_2MHz()

%% DATA FILE NAME

SysParam.filename = 'Z:\ForMATLAB\data\RefAuGlass2MHz@2MHz_011123_205603_FIN.mat';
...
% new try - Nov. 13; to fit: lambda_SiO2 + h_Au: didn't work.
% better: fit h_Au and lambda_Ti (as an interfacial layer).
SysParam.Lambda  = [315 0.3 1.3];  % Thermal conductivities (W m^-1 K^-1)
SysParam.C       = [2.49 2.63 1.86]*1e6;  % Volumetric heat capacities (J m^-3 K^-1)
SysParam.h       = [52 5 1e6]*1e-9;  % Thicknesses (m)  
...
% TEMP: vary L_SiO2 and plot the resulting h_Au and L_Ti.
SysParam.Lambda  = [345 0.26045 1.3];  % Thermal conductivities (W m^-1 K^-1)
SysParam.C       = [2.49 2.63 1.86]*1e6;  % Volumetric heat capacities (J m^-3 K^-1)
SysParam.h       = [51.41 5 1e6]*1e-9;  % Thicknesses (m)
