function [SysParam] = Parameter_Test_Si()


%% DATA FILE NAME
SysParam.filename = 'Si_N3_testdata_SHIFTED.mat';
%% SAMPLE PROPERTIES (starting with top layer )
SysParam.Lambda  = [237 124];  % Thermal conductivities (W m^-1 K^-1)
SysParam.C       = [2.4 1.63]*1e6;  % Volumetric heat capacities (J m^-3 K^-1)
SysParam.h       = [75 1e6]*1e-9;  % Thicknesses (m)   
...
SysParam.r_pump   = 7e-6;  % Pump 1/e^2 radius (m)
SysParam.r_probe  = 10e-6;  % Probe 1/e^2 radius, (m)
SysParam.tau_rep  = 1/76e6;   % Laser repetition period (s) 
SysParam.P_pump   = 0.8*0.3*30e-3;   % absorbed pump power (transmission of objective * absorbance * pump power) 
SysParam.P_probe  = 0.8*0.3*17.5e-3;   % absorbed probe power (transmission of objective * absorbance * probe power); assumes AF chopper is OFF!  If not, then you need to multiply the probe power by 2.
