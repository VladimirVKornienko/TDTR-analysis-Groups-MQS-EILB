function dTss = SS_Heating_vH4(matparams,r_pump,r_probe,absC,A_tot,intscheme,nnodes)
%SS_Heating_vH4 - Calculates the steady state heating of an anisotropic 
%multilayer stack. Can handle bidirectional heat flow.
%
% Syntax:  SS_Heating_vH4(matparams,r_pump,r_probe,absC,A_tot,intscheme,nnodes)
%
% Inputs:
...
%                 isotropic
%[See INITIALIZE_CELLPARAMS_vH4.m for more details.]
...
%    --
%
% Other m-files required: TDTR_TEMP_vH4.m
% Subfunctions: none
...
r = sqrt(r_pump(1)^2+r_probe^2)/sqrt(2);
...
if BI
    ...
    switch intscheme
        case 0 % Legendre-Gauss
            ...
            Iu = TDTR_TEMP_vH4(kvect,f,LCTEu,r_pump,r_probe,A_abs);
            ...
            Id = TDTR_TEMP_vH4(kvect,f,LCTEd,r_pump,r_probe,A_abs);
            [Nk,Nf,Nt] = size(Id); % get 3D matrix dimensions (for variable r_pump)
            dTd = reshape(weights' * Id(:,:), 1, Nf, Nt);   % (for variable r_pump)
            %dTd = weights'*Id;
        case 1 % rombint
            dTu=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,f,LCTEu,r_pump,r_probe,A_abs),kmin,kmax,1);
            dTd=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,f,LCTEd,r_pump,r_probe,A_abs),kmin,kmax,1);
    ...
    switch intscheme
        case 0 % Legendre-Gauss
            I = TDTR_TEMP_vH4(kvect,f,LCTE,r_pump,r_probe,A_abs);
            ...
            %dTss = weights'*I;
        case 1 % rombint
            dTss=rombint_VV3(@(kvect) TDTR_TEMP_vH4(kvect,f,LCTE,r_pump,r_probe,A_abs),kmin,kmax,1);
        case 2 % Simpson
            kvect = linspace(kmin,kmax,nnodes)';
            I = TDTR_TEMP_vH4(kvect,f,LCTE,r_pump,r_probe,A_abs);
