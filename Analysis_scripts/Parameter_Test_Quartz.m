function [SysParam] = Parameter_Test_Quartz()

%% DATA FILE NAME
%sio2 test N2
SysParam.filename = 'Quartz_N4_testdata_SHIFTED.mat';
%% SAMPLE PROPERTIES (starting with top layer )
SysParam.Lambda  = [237  1.4];  % Thermal conductivities (W m^-1 K^-1)
SysParam.C       = [2.4 1.88]*1e6;  % Volumetric heat capacities (J m^-3 K^-1)
SysParam.h       = [88 1 1e6]*1e-9;  % Thicknesses (m)  %67nm Al 
...
SysParam.P_pump   = 0.8*0.3*37e-3;   % absorbed pump power (transmission of objective * absorbance * pump power) 
